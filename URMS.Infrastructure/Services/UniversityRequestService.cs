...
public class UniversityRequestService : IUniversityRequestService
{
    ...
    public async Task<UniversityRequestResponseDto> OverrideStatusByAdminAsync(int requestId, string adminId, AdminOverrideRequestDto dto)
    {
        var request = await _context.UniversityRequests
            .Include(r => r.Student).ThenInclude(u => u.Student)
            .Include(r => r.Advisor)
        ...
        return MapToDto(request, request.Student, request.Advisor ?? adminUser, request.Staff ?? adminUser);
    }
